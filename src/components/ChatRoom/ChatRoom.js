  ...
  return (
    <div className="py-4 m-5 w-50 shadow bg-white text-dark border rounded container">
      <div className="text-center px-3 ">
        <h4 className="text-warning mb-4 fw-bold">
          LineupApp
          <span className="fs-4"> {data?.room} </span>
          Chat Room
        </h4>
      </div>
      <div
          ...
          return data.name === msg.name ? (
            <div className="row justify-content-end pl-5 ">
              <div className="d-flex flex-column align-items-end m-2 shadow p-2 bg-info border rounded w-auto">
                <div>
                  <strong className="m-1 secondary">{msg.name}</strong>
                  <small className="text-muted m-1">
                    <Moment fromNow>{msg.time}</Moment>
                  </small>
                </div>
                <p className="m-1 fs-6 text-white">{msg.msg}</p>
              </div>
            </div>
          ) : (
            <div className="row justify-content-start">
              <div className="d-flex flex-column m-2 p-2 shadow bg-secondary border rounded w-auto">
                <div>
                  <strong className="m-1 text-warning">{msg.name}</strong>
                  <small className="text-muted m-1">
                    <Moment fromNow>{msg.time}</Moment>
                  </small>
                </div>
